...
if(isset($_POST['btn_action']))
{
	...
	{
		$query = "
		INSERT INTO subtblinventory_parts (user_id, inventory_order_total, inventory_order_date, inventory_order_name, inventory_order_address, payment_status, inventory_order_status, inventory_order_created_date) 
		...
		if(isset($inventory_order_id))
		{
			...
			for($count = 0; $count<count($_POST["product_id"]); $count++)
			{
				...
				$sub_query = "
				INSERT INTO subtblinventory_parts_product (inventory_order_id, product_id, quantity, price, tax) VALUES (:inventory_order_id, :product_id, :quantity, :price, :tax)
				";
	...
	{
		$query = "
		SELECT * FROM subtblinventory_parts WHERE inventory_order_id = :inventory_order_id
		";
		$statement = $connect->prepare($query);
		$statement->execute(
			array(
	...
	{
		$delete_query = "
		DELETE FROM subtblinventory_parts_product 
		WHERE inventory_order_id = '".$_POST["inventory_order_id"]."'
		";
	...
	if($_POST['btn_action'] == 'delete')
	{
		...
		$subquery = "
		SELECT * FROM subtblinventory_parts_product WHERE inventory_order_id = :inventory_order_id;
		";
		...
		{
			$sum = get_only_one($row['product_id'],$connect)-$row['quantity'];
			$query_updateProd = "
						UPDATE subtblproduct 
						set product_quantity = :product_quantity 
						WHERE product_id = :product_id
						";
						$statement = $connect->prepare($query_updateProd);
						$statement->execute(
							array(
						...
						$resultUpdate = $statement->fetchAll();
		}
	...
	if($_POST['btn_action']=='finish')
	{
		$query_update = "
						...
						WHERE inventory_order_id = :inventory_order_id
						";
						$statement = $connect->prepare($query_update);
						$statement->execute(
							array(
								':inventory_order_status'		=>	"finished",
								':inventory_order_id'			=>	$_POST['inventory_order_id']
							)
						);
						$resultUpdate = $statement->fetchAll();
		...
		$subquery_update = "
						...
						";
						$statement = $connect->prepare($subquery_update);
						$statement->execute(
							array(
								':status'	=>	"available",
						...
						$resultUpdatesub = $statement->fetchAll();

		echo 'Order status change to Delivered';
	}
}

function get_only_one($product_id,$connect)
{
	$query = "
		SELECT * FROM subtblproduct WHERE product_id = :product_id LIMIT 1;
		";
		$statement = $connect->prepare($query);
		$statement->execute(
			array(
