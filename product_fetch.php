...
foreach($result as $row)
{
	...
	$sub_array[] = '<button type="button" name="RemoveStocks" id="'.$row["product_id"].'" class="btn btn-primary btn-xs RemoveStocks" data-status="'.$row["product_status"].'" data-oldstocks="'.$row["product_quantity"].'">Deduct Stock</button>';
	$sub_array[] = '<button type="button" name="addingstock" id="'.$row["product_id"].'" class="btn btn-default btn-xs addingstock" data-status="'.$row["product_status"].'" data-oldstocks="'.$row["product_quantity"].'">Add Stock</button>';
	$data[] = $sub_array;
}
...
function get_total_all_records($connect)
{
	$statement = $connect->prepare('SELECT * FROM subtblproduct
		...
		INNER JOIN subtblcategory ON subtblcategory.category_id = subtblproduct.category_id 
		INNER JOIN tblaccountinfo ON tblaccountinfo.user_id = subtblproduct.product_enter_by 
		WHERE subtblproduct.product_quantity >subtblproduct.product_low_stock');
	$statement->execute();
